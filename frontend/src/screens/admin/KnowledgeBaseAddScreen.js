...
const KnowledgeBaseAddScreen = ({history}) => {
    ...
    const { success: successCreate } = useSelector( state => state.knowledgeBaseCreate)

    useEffect(() => {
        if (successCreate) {
            toast.success('Publishing Knowledge Base..',{
            position: "bottom-right",});
            setTimeout(() => {
                history.push(`/admin/knowledge-base`)
              }, 5000);
            dispatch({type: KNOWLEDGE_BASE_CREATE_RESET})
        }
        dispatch(listUsers())
